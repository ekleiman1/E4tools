...
```
# E4tools
Tested with TravisCI: [![Build Status](https://travis-ci.org/ekleiman1/E4tools.svg?branch=master)](https://travis-ci.org/ekleiman1/E4tools)  
E4tools is a set of tools that you can use to automate your workflow for analyzing data that comes from the Empatica E4. You can run these functions consecutively. 
...
E4tools now comes with the **E4.Step0.FileHelper** function. This function will not only cause other functions to require fewer arguments, but will also automatically create a folder structure that is consistent across all steps. When you run this function, you will only need to specify the location of your ZIP files (see step 1 above) and where you want your output folder to go. Once you do this, for all other functions, you will not need to specify any file locations. Instead, you will just specify "helper" for the argument (e.g.,ziplocaton="helper"), which will tell E4tools to use the file structure specified by the File Helper function. 

**What does the file structure look like?**  
The file structure is organized into five output folders, each representing the type of output data. 

* **raw_data**, which contains raw data that are extracted from ZIP files (e.g., raw EDA)
* **metadata**, which contains metadata (e.g., E4 sessions summaries)
* **matched_data**, which contains data that are matches to events (e.g., button pressess)
* **binned_data**, which contains data that have been binned
* **plots**, which contains plots generated from the data (e.g., output from the diagnostics functions)


## What can you do now with E4tools?
There are currently tools to work EDA and accelerometer data.
...
**E4.Acc_Process.part1.ExtractRawAcc**, which allows you extract acceleromter data. It will output raw acceleromter data (x,y,z).

**E4.Acc_Process.part2.Filter_ConvertAcc**, which allows you to filter acceleromter data (based on the EDA signal) and add metrics like g and the normalized Euclidian distance from origin vector.

### The temperature pipeline

**E4.Temp.part1.extract_raw_temp**, which allows you to create one file per participant that includes raw temperature data.

**E4.Temp.part2.bin_temp**, which allows you to place the temperature data in bins of X minutes in length.
